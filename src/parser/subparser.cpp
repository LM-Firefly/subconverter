...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "vless"_hash:
            ...
            vlessConstruct(node, VLESS_DEFAULT_GROUP, ps, server, port, type, id, net, "", flow, mode, path, host, "", tls, public_key, short_id, fp, sni, alpnList, packet_encoding, encryption, vless_udp, tfo, scv, tribool(), "", v2ray_http_upgrade, v2ray_http_upgrade_fast_open, tribool(packet_addr_enabled), tribool(xudp_enabled));
            break;
        case "tuic"_hash:
...
void explodeStdVLESS(std::string vless, Proxy &node)
{
    ...
    vlessConstruct(node, VLESS_DEFAULT_GROUP, remarks, add, port, type, uuid, net, "", flow, mode, path, host, "", tls, public_key, short_id, fingerprint, sni, alpnList, packet_encoding, encryption, vless_udp, tfo, scv, tribool(), "", v2ray_http_upgrade, v2ray_http_upgrade_fast_open, tribool(), tribool());
    return;
}
            ...
            if(singboxNode.HasMember("type") && singboxNode["type"].IsString())
            {
                ...
                switch(hash_(proxytype))
                {
                    ...
                    case "vless"_hash:
                        ...
                        vlessConstruct(node, group, ps, server, port, type, id, net, "", flow, mode, path, host, "", tls, public_key, short_id, fingerprint, sni, alpnList, packet_encoding, encryption, udp, tfo, scv, tribool(), underlying_proxy, tribool(), tribool(), tribool(), tribool());
                        break;
                    case "tuic"_hash:
                        group = TUIC_DEFAULT_GROUP;
