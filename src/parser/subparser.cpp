...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "openvpn"_hash:
            ...
            openvpnConstruct(node, group, ps, server, port, ovpn_proto, ovpn_dev, ovpn_cipher, ovpn_auth, ovpn_ca, ovpn_cert, ovpn_key, ovpn_tls_crypt, mtu, udp, underlying_proxy, remote_dns_resolve, dns_server);
            if(!user.empty())
                node.Username = user;
            if(!password.empty())
                node.Password = password;
            if(!ovpn_comp_lzo.empty())
                node.CompLZO = ovpn_comp_lzo;
            if(!ovpn_ping.empty())
                node.OpenVPNPing = to_int(ovpn_ping);
            if(!ovpn_ping_restart.empty())
                node.OpenVPNPingRestart = to_int(ovpn_ping_restart);
            break;
        case "hysteria"_hash:
        ...
        case "hysteria2"_hash:
            ...
            hysteria2Construct(node, group, ps, server, port, ports, up, down, password, auth, obfs, obfs_password, sni, fingerprint, ca, ca_str, cwnd, hop_interval, ech_enable, ech_config, initial_stream_receive_window, max_stream_receive_window, initial_connection_receive_window, max_connection_receive_window, udp, tfo, scv, underlying_proxy, alpnList, to_int(obfs_min_packet_size), to_int(obfs_max_packet_size));
            if(!udp_mtu.empty())
                node.UdpMTU = to_int(udp_mtu);
        ...
        case "gost-relay"_hash:
            ...
            commonConstruct(node, ProxyType::GostRelay, group, ps, server, port, udp, tfo, scv, tribool(), underlying_proxy);
            node.ServerName = sni;
            node.TLSSecure = tls == "true";
            node.Username = user;
            node.Password = password;
        ...
        case "anytls"_hash:
            ...
            anyTLSConstruct(node, ANYTLS_DEFAULT_GROUP, ps, server, port, password, sni, alpnList, fingerprint, idle_session_check_interval, idle_session_timeout, min_idle_session, tfo, scv, tribool(), underlying_proxy, multiplexing, ip_version, reuse);
            node.UDP = udp;
            break;
        case "mieru"_hash:
...
void explodeStdOpenVPN(std::string openvpn, Proxy &node)
{
    ...
    openvpnConstruct(node, OPENVPN_DEFAULT_GROUP, remarks, hostname, port, proto, dev, cipher, auth, ca, cert, key, tls_crypt, mtu, tribool(), "", remote_dns_resolve_flag, dns_arr);
    if(!username.empty())
        node.Username = username;
    if(!password.empty())
        node.Password = password;
    ...
    if(!ping_str.empty())
        node.OpenVPNPing = to_int(ping_str);
    if(!ping_restart_str.empty())
        node.OpenVPNPingRestart = to_int(ping_restart_str);
}
