...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "vless"_hash:
            ...
            tls = safe_as<std::string>(singleproxy["tls"]) == "true" ? "tls" : "";
            if(singleproxy["reality-opts"].IsDefined() && singleproxy["reality-opts"].IsMap())
            {
            ...
            vlessConstruct(node, group, ps, server, port, type, id, aid, net, cipher, flow, mode, path, host, "", tls, pbk, sid, fp, sni, alpnList, packet_encoding, udp, tfo, v2ray_http_upgrade);
            break;
        case "tuic"_hash:
...
{
    std::string add, port, type, id, aid, net, flow, cipher, pbk, sid, fp, mode, path, host, tls, remarks, sni;
    std::string addition;
    ...
    vlessConstruct(node, XRAY_DEFAULT_GROUP, remarks, add, port, type, id, aid, net, cipher, flow, mode, path, host, "", tls, pbk, sid, fp, sni, alpnList, packet_encoding);
    return;
}
            ...
            if(singboxNode.HasMember("type") && singboxNode["type"].IsString())
            {
                ...
                switch(hash_(proxytype))
                {
                    ...
                    case "vless"_hash:
                        ...
                        vlessConstruct(node, group, ps, server, port, type, id, aid, net, cipher, flow, mode, path, host, "", tls, pbk, sid, fp, sni, alpnList, packet_encoding, udp, underlying_proxy);
                        break;
                    case "tuic"_hash:
                        group = TUIC_DEFAULT_GROUP;
