...
void hysteria2Construct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &server, const std::string &port, const std::string &ports, const std::string &up, const std::string &down, const std::string &password, const std::string &obfs, const std::string &obfs_password, const std::string &sni, const std::string &fingerprint, const std::string &alpn, const std::string &ca, const std::string &ca_str, const std::string &cwnd, const std::string &hop_interval, const std::string &ech_enable, const std::string &ech_config, const std::string &initial_stream_receive_window, const std::string &max_stream_receive_window, const std::string &initial_connection_receive_window, const std::string &max_connection_receive_window, tribool tfo, tribool scv, const std::string &underlying_proxy) {
...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "hysteria2"_hash:
            ...
            hysteria2Construct(node, group, ps, server, port, ports, up, down, password, obfs, obfs_password, sni, fingerprint, ca, ca_str, cwnd, alpn, hop_interval, ech_enable, ech_config, initial_stream_receive_window, max_stream_receive_window, initial_connection_receive_window, max_connection_receive_window, tfo, scv, underlying_proxy);
            break;
        case "vless"_hash:
            ...
            vlessConstruct(node, VLESS_DEFAULT_GROUP, ps, server, port, type, id, aid, net, "auto", flow, mode, path, host, "", tls, public_key, sid, fingerprint, sni, alpnList, packet_encoding, udp, tfo, scv, tribool(), "", v2ray_http_upgrade);
            break;
        case "tuic"_hash:
