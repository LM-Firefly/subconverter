...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    std::string key, aead, padding_min, padding_max, seed, ascii, http_mask, enable_pure_downlink, http_mask_strategy, custom_table; // sudoku
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "sudoku"_hash:
            ...
            singleproxy["http-mask-strategy"] >>= http_mask_strategy;
            ...
            if(!http_mask_strategy.empty())
                node.HTTPMaskStrategy = http_mask_strategy;
