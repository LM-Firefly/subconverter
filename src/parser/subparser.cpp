...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    std::string obfs_password, cwnd, ech_enable, ech_config, initial_stream_receive_window, max_stream_receive_window, initial_connection_receive_window, max_connection_receive_window, udp_mtu; //hysteria2
    ...
    std::string ovpn_proto, ovpn_dev, ovpn_cipher, ovpn_auth, ovpn_ca, ovpn_cert, ovpn_key, ovpn_tls_crypt; // openvpn
...
void explodeStdOpenVPN(std::string openvpn, Proxy &node)
{
    ...
    std::string username, password, hostname, port, proto, dev, cipher, auth, ca, cert, key, tls_crypt, mtu, dns, remarks, addition;
