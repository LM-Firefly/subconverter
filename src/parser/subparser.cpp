...
{
    commonConstruct(node, ProxyType::VLESS, group, remarks, add, port, tribool(), tfo, scv, tls13, underlying_proxy);
...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
                ...
                if(singleproxy["plugin"].IsDefined())
                {
                    ...
                    case "v2ray-plugin"_hash:
                        ...
                        if(singleproxy["plugin-opts"].IsDefined())
                        {
                            ...
                            std::string fingerprint, server_name, ech_config, ech;
                            ...
                            if(singleproxy["plugin-opts"]["ech-config"].IsDefined())
                            {
                                singleproxy["plugin-opts"]["ech-config"] >>= ech_config;
                                if(!ech_config.empty())
                                    pluginopts += "ech-config=" + ech_config + ";";
                            }
                            if(singleproxy["plugin-opts"]["ech"].IsDefined())
                            {
                                singleproxy["plugin-opts"]["ech"] >>= ech;
                                if(!ech.empty())
                                    pluginopts += "ech=" + ech + ";";
                            }
        ...
        case "trojan"_hash:
            ...
            trojanConstruct(node, group, ps, server, port, password, "", "", "", fp, sni, alpnList, true, udp, tfo, scv, tribool(), underlying_proxy);
            break;
        case "snell"_hash:
            group = SNELL_DEFAULT_GROUP;
        ...
        */
        ...
        default:
            ...
            case "vmess"_hash: //quantumult x style vmess link
                ...
                vmessConstruct(node, V2RAY_DEFAULT_GROUP, remarks, server, port, "", id, aead, net, method, path, host, "", tls, "", std::vector<std::string>{}, udp, tfo, scv, tls13, underlying_proxy, fp, client_id);
                break;
