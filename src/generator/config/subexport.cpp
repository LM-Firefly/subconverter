...
void proxyToClash(std::vector<Proxy> &nodes, YAML::Node &yamlnode, const ProxyGroupConfigs &extra_proxy_group, bool clashR, extra_settings &ext)
{
    ...
    for(Proxy &x : nodes)
    {
        ...
        case ProxyType::Shadowsocks:
            ...
            case "v2ray-plugin"_hash:
                ...
                if(!x.EchConfig.empty())
                    singleproxy["plugin-opts"]["ech-config"] = x.EchConfig;
                if(!x.EchEnable.is_undef())
                    singleproxy["plugin-opts"]["ech"] = x.EchEnable.get();
                ...
                headers_v2ray = getUrlArg(pluginopts, "headers");
                if(!headers_v2ray.empty())
                {
                    string_size pos = 0, next_pos = 0;
                    while((next_pos = headers_v2ray.find(";", pos)) != std::string::npos)
                    {
                        std::string header = headers_v2ray.substr(pos, next_pos - pos);
                    ...
                    std::string header = headers_v2ray.substr(pos);
                    string_size sep_pos = header.find("=");
                    if(sep_pos != std::string::npos)
                    {
                        std::string key = header.substr(0, sep_pos);
        ...
        case ProxyType::VMess:
            ...
            singleproxy["cipher"] = x.EncryptMethod == "none" ? "auto" : x.EncryptMethod;
            singleproxy["tls"] = x.TLSSecure;
...
std::string proxyToSingle(std::vector<Proxy> &nodes, int types, extra_settings &ext)
{
    ...
    for(Proxy &x : nodes)
    {
        ...
        std::string &hostname = x.Hostname, &sni = x.ServerName, &password = x.Password, &method = x.EncryptMethod, &plugin = x.Plugin, &pluginopts = x.PluginOption, &protocol = x.Protocol, &protoparam = x.ProtocolParam, &flow = x.Flow, &public_key = x.PublicKey, &sid = x.ShortID, &fingerprint = x.Fingerprint, &packet_encoding = x.PacketEncoding, &fake_type = x.FakeType, &mode = x.GRPCMode, &obfs = x.OBFS, &obfsparam = x.OBFSParam, &obfsPassword = x.OBFSPassword, &id = x.UserId, &transproto = x.TransferProtocol, &host = x.Host, &tls = x.TLSStr, &path = x.Path, &faketype = x.FakeType, &ports = x.Ports;
        bool &tlssecure = x.TLSSecure;
