            ...
            if(!transproto.empty())
            {
                ...
                switch(hash_(transproto))
                {
                    ...
                    case "quic"_hash:
                        if(!fake_type.empty())
                            proxyStr += "&headerType=" + urlEncode(fake_type);
                        proxyStr += "&quicSecurity=" + urlEncode(host.empty() ? sni : host);
                        if(!path.empty())
                            proxyStr += "&key=" + urlEncode(path);
                        break;
                    default:
                        break;
                }
    ...
    for (Proxy &x : nodes)
    {
        ...
        switch (x.Type)
        {
            ...
            case ProxyType::VLESS:
            {
                ...
                if(!packet_encoding.empty())
                    proxy.AddMember("packet_encoding", rapidjson::StringRef(packet_encoding.c_str()), allocator);
                if(!x.Flow.empty())
                ...
                case "ws"_hash:
                    ...
                    vlesstransport.AddMember("path", rapidjson::StringRef(x.Path.empty() ? "/" : x.Path.c_str()), allocator);
                    if(!x.Host.empty())
