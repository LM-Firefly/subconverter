        ...
        case ProxyType::VLESS:
            ...
            switch(hash_(x.TransferProtocol))
            {
                ...
                case "xhttp"_hash:
                    ...
                    if(!x.XHTTPDownloadNoGRPCHeader.is_undef())
                        singleproxy["xhttp-opts"]["download-settings"]["no-grpc-header"] = x.XHTTPDownloadNoGRPCHeader.get();
                    if(!x.XHTTPDownloadXPaddingBytes.empty())
                        singleproxy["xhttp-opts"]["download-settings"]["x-padding-bytes"] = x.XHTTPDownloadXPaddingBytes;
                    if(!x.XHTTPDownloadScMaxEachPostBytes.empty())
                        singleproxy["xhttp-opts"]["download-settings"]["sc-max-each-post-bytes"] = x.XHTTPDownloadScMaxEachPostBytes;
                    if(!x.XHTTPDownloadScMaxBufferedPosts.empty())
                        singleproxy["xhttp-opts"]["download-settings"]["sc-max-buffered-posts"] = x.XHTTPDownloadScMaxBufferedPosts;
                    if(!x.XHTTPDownloadScMinPostsIntervalMs.empty())
                        singleproxy["xhttp-opts"]["download-settings"]["sc-min-posts-interval-ms"] = x.XHTTPDownloadScMinPostsIntervalMs;
                    ...
                    if(!x.XHTTPDownloadRealityPublicKey.empty() || !x.XHTTPDownloadRealityShortID.empty() || !x.XHTTPDownloadRealitySpiderX.empty() || !x.XHTTPDownloadRealitySupportX25519Mlkem768.is_undef())
                    {
                        ...
                        if(!x.XHTTPDownloadRealityShortID.empty())
                        {
                            ...
                            singleproxy["xhttp-opts"]["download-settings"]["reality-opts"]["short-id"].SetTag("str");
                        }
