...
std::string proxyToSingle(std::vector<Proxy> &nodes, int types, extra_settings &ext)
{
    ...
    for(Proxy &x : nodes)
    {
        ...
        case ProxyType::Sudoku:
            ...
            if(!x.AEAD.empty())
                proxyStr += (proxyStr.find('?') == std::string::npos ? "?" : "&") + std::string("aead=") + urlEncode(x.AEAD);
            if(x.PaddingMin > 0)
                proxyStr += (proxyStr.find('?') == std::string::npos ? "?" : "&") + std::string("padding_min=") + std::to_string(x.PaddingMin);
            if(x.PaddingMax > 0)
                proxyStr += (proxyStr.find('?') == std::string::npos ? "?" : "&") + std::string("padding_max=") + std::to_string(x.PaddingMax);
            if(!x.Seed.empty())
                proxyStr += (proxyStr.find('?') == std::string::npos ? "?" : "&") + std::string("seed=") + urlEncode(x.Seed);
            if(!x.TableType.empty())
                proxyStr += (proxyStr.find('?') == std::string::npos ? "?" : "&") + std::string("ascii=") + urlEncode(x.TableType);
            if(!x.HTTPMask.is_undef())
                proxyStr += (proxyStr.find('?') == std::string::npos ? "?" : "&") + std::string("http_mask=") + (x.HTTPMask.get() ? "1" : "0");
            proxyStr += "#" + urlEncode(remark);
            break;
        case ProxyType::Hysteria2:
            if(!hysteria2)
