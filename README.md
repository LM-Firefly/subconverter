...
3. When target type is `mixed`, it outputs all supported single proxy links (including WireGuard) as a Base64-encoded plain subscription.

4. When target type is `auto`, the output target type is automatically determined based on the request's `User-Agent`.

5. You can use `&ua=` to set a custom User-Agent for downloading subscriptions, and `&agekey=` to provide an X25519 private key for decrypting age-encrypted subscriptions.
